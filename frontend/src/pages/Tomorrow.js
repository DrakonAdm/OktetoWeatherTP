...
import {getAd, getClothes, getWeatherMain, getWeatherTomorrow} from "../http/weatherApi";
import Loader from "../components/Loader/Loader";

const Tomorrow = observer(() => {
    ...
    useEffect(() => {
        getWeatherTomorrow("tomorrow", user.chooseCity).then(day => {
            setData(day.data.results[0])
        });
        // getAd("tomorrowPage").then(page => data.setPage(page))
    }, [])
                        ...
                        <Row>
                            <Col md={9}>
                                <Card style={{width: 800, borderWidth: 0, backgroundColor: "transparent", }} className="p-4 d-flex">
                                    <h2>Прогноз погоды на завтра, {data?.city__city} </h2>
                                    <ButtonGroup className="mt-3 d-flex">
                                        <Button className="button_menu" variant="outline-dark" href={HOME_ROUTE}>Сегодня</Button>
                                        <Button className="button_menu_active" variant="outline-dark" href={TOMORROW_ROUTE}>Завтра</Button>
                            ...
                            <Col>
                                <Card style={{left: 50, top: 130, height: 400, width:300, borderWidth: 0, backgroundColor: "transparent",}} className="align-items-center text-center">
                                    <Card.Body>
                                        <Card.Img width={300} height={400} src={`${process.env.REACT_APP_API_URL}advertisementGET/?short=date`}/>
                                    </Card.Body>
                                </Card>
                            </Col>
                        </Row>
